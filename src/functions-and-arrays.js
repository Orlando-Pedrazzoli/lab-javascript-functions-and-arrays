...
const longest = findLongestWord(wordsArray);
console.log(longest);
...
console.log(averageNumbers([2, -4, 6, 8]));
...
function averageWordLength(arr) {
  ...
  const totalLength = arr.reduce(
    (accumulator, word) => accumulator + word.length,
    0
  );
...
function uniquifyArray(inputArray) {
  ...
  const uniqueArray = inputArray.filter((element, index, self) => {
...
console.log(uniquifyArray(['a', 'b', 'c']));
...
function doesWordExist(arr, wordToFind) {
  ...
  if (arr.length === 1 && arr[0] === wordToFind) {
    return true;
  }
...
console.log(doesWordExist([], 'something'));
...
console.log(howManyTimes(words, 'matter')); // 5
console.log(howManyTimes(words, 'nonexistent')); // 0
